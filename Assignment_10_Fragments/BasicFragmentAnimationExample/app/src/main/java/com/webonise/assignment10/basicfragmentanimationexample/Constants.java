package com.webonise.assignment10.basicfragmentanimationexample;

/**
 * Created by webonise on 14/8/15.
 */
public class Constants {

    public static final int DB_VERSION = 1;
    public static final String DATABASE_NAME = "person_info_database";
    public static final String TABLE_PERSON_DETAILS = "person_details";
    public static final String COLUMN_ID = "id";
    public static final String COLUMN_NAME = "name";
    public static final String COLUMN_AGE = "age";
    public static final String COLUMN_WEIGHT = "weight";
    public static final String COLUMN_HEIGHT = "height";
    public static final int ZERO = 0;
    public static final int FIRST_INDEX = 1;
    public static final int SECOND_INDEX = 2;
    public static final int THIRD_INDEX = 3;
    public static final int FORTH_INDEX = 4;
    public static final Double FLOAT_ZERO = 0.0;
    public static final String NO_DATA = "No Data Present";
}
